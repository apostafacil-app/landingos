/**
 * Biblioteca de ícones SVG estilo Lucide — substitui emoji "infantil" por
 * iconografia profissional consistente.
 *
 * Cada ícone é um SVG inline 24x24 stroke=2 lineCap=round. Mapa emoji→icon
 * tenta inferir qual ícone usar quando o agente gera emoji.
 */

/** Catálogo — ícones Lucide-style desenhados manualmente */
const ICONS: Record<string, string> = {
  ...
  bar_chart:  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" x2="12" y1="20" y2="10"/><line x1="18" x2="18" y1="20" y2="4"/><line x1="6" x2="6" y1="20" y2="16"/></svg>',
  ...
  bolt:       '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>',
  ...
  thumbs_up:  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7 10v12"/><path d="M15 5.88 14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.76a2 2 0 0 0 1.79-1.11L12 2a3.13 3.13 0 0 1 3 3.88Z"/></svg>',
...
/** Mapeia emoji → ícone correspondente (com heurística de match aproximado) */
const EMOJI_MAP: Record<string, keyof typeof ICONS> = {
  '⚡': 'zap',         '🚀': 'rocket',     '🛡️': 'shield',    '🛡': 'shield',
  '✅': 'check',       '✓': 'check',       '⭐': 'star',       '🌟': 'star',
  '🏆': 'trophy',      '💎': 'diamond',    '⏱️': 'clock',     '⏱': 'clock',
  '⏰': 'clock',       '📊': 'bar_chart',  '🎯': 'target',     '✨': 'sparkles',
  '👥': 'users',       '👍': 'thumbs_up',  '❤️': 'heart',     '💬': 'message',
  '💰': 'dollar',      '💵': 'dollar',     '🌐': 'globe',      '📦': 'package',
  '📄': 'file',        '📋': 'file',       '⚙️': 'settings',  '🔄': 'refresh',
  '🏅': 'award',       '🥇': 'award',      '🔒': 'lock',       '🔐': 'lock',
}

/**
 * Detecta o primeiro emoji/símbolo no início de um texto e devolve o SVG
 * inline equivalente (Lucide-style). Se não houver mapeamento, retorna null.
 *
 * @param color - cor stroke do ícone (CSS color)
 * @returns string com SVG inline ou null
 */
export function emojiToSvg(text: string, color: string): string | null {
  const trimmed = text.trimStart()
  // Tenta extrair emoji multi-codepoint primeiro (ex: 🛡️ = 2 codepoints)
  for (let len = 4; len > 0; len--) {
    const candidate = trimmed.slice(0, len)
    const iconKey = EMOJI_MAP[candidate]
    if (iconKey && ICONS[iconKey]) {
      return ICONS[iconKey].replace('currentColor', color).replace('currentColor', color)
    }
  }
  return null
}

/** Remove o emoji do início de um texto pra mostrar só o conteúdo. */
export function stripLeadingEmoji(text: string): string {
  return text.replace(/^[\p{Emoji_Presentation}\p{Extended_Pictographic}️]\s*/u, '').trim()
}

/** Retorna ícone SVG por nome direto (pra usar em templates). */
